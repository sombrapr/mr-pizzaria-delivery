...
async function buildSiteOrderItems(rawItems) {
  ...
  for (const raw of rawItems) {
    ...
    if (raw?.kind === "combo") {
      ...
      const borderKey = cleanSiteText(raw.borderKey, 40) || "none";
      const border = CONFIG.borders[borderKey];
      if (!border) throw new Error("Escolha uma opção de borda válida.");
      const borderExtra = Number(border.G || 0);
      const unit = combo.price + borderExtra;
      const flavorLabel = flavor2 ? `½ ${flavor1} / ½ ${flavor2}` : flavor1;
      const borderLabel = borderKey !== "none" ? ` — ${border.name}` : "";
      built.push({
        type: "product",
        category: "Combos",
        name: `${combo.name} — Pizza grande ${flavorLabel}${borderLabel}`,
        qty,
        unit,
        total: unit * qty,
